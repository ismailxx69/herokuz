...
if(!['796424637114351636'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) 
return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
const kadınrol = message.guild.roles.cache.find(r => r.id === '796424653529677824') //kadınrol isimini değişme
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '796466258013847592')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '796424667019477032')
...
if(member.id === message.author.id) return message.channel.send('Kendine yetki veremezsin.')
if(member.id === client.user.id) return message.channel.send('Bota yetki veremezsin.')
if(member.id === message.guild.OwnerID) return message.channel.send('Sunucu sahibine yetki veremezsin.')
...
.setDescription(`
• ${member}, ${message.author} Tarafından Yetki Verildi.
• ${kadınrol}, ${kadınrol2} Rolleri Verildi.
• İsmi \`${tag} ${name} | ${age}\` Olarak Güncellendi.`) 
.setFooter(`${message.author.username} Başarıyla Yetkiyi Verdim.`)
.setColor("0x2f3136")
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ['yetkili', 'yetkilial', 'yetki', '1', 'boş'],
    permLevel: 0
  }
