...
if(!['797192784984670278'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) 
return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
const erkekrol = message.guild.roles.cache.find(r => r.id === '797192836814340137') //erkekrol ismini değişmeyin
const erkekrol2 = message.guild.roles.cache.find(r => r.id === '797192839196704858')
const erkekrol3 = message.guild.roles.cache.find(r => r.id === '797192841289793604')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '797192954304266280')
...
const sph = new Discord.MessageEmbed()
.setDescription(`• ${member} **Aramıza Katıldı Toplam** : \`${message.guild.memberCount}\` **Kişiye Ulaştık ! Tekrardan Hoşgeldin Dostum.**`)
.setColor("GREEN")
.setFooter(footer.sph)

const embed = new Discord.MessageEmbed()
.setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }))
.setDescription(`
• ${member}, ${message.author} Tarafından Kayıt Edildi.
• ${erkekrol}, ${erkekrol2} , ${erkekrol3} Rolleri Verildi.
• İsmi \`${tag} ${name} | ${age}\` Olarak Güncellendi.`) 
.setFooter(`${message.author.username} Toplam ${alldata} Kayıta Sahip.`)
.setColor("0x2f3136")
message.channel.send(embed)
client.channels.cache.get("784147484628877382").send(sph).then(erdemm => erdemm.delete({timeout : 10000})) 
